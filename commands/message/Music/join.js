...
module.exports = {
  ...
  execute: async (message, args, client, prefix) => {
    ...
    if (!message.guild.members.me.permissions.has([PermissionFlagsBits.Connect, PermissionFlagsBits.Speak])) return message.channel.send({embeds: [new EmbedBuilder().setColor(client.embedColor).setDescription(`<:Fauna_Cross:1097716530025861190> | I don't have enough permissions to execute this command! Please give me permission to \`CONNECT\` or \`SPEAK\`.`)]});
    ...
    const emojiJoin = message.client.emoji.join;
     ...
     player = message.client.manager.create({
      ...
      let thing = new EmbedBuilder()
        ...
        .setDescription(`<:Fauna_Tick:1097715832852852806> | **Joined the voice channel.**\nJoined <#${channel.id}> and bound to <#${message.channel.id}>`)
