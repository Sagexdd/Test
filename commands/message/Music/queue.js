  ...
  execute: async (message, args, client, prefix) => {
    ...
    if (player.queue.length === "0" || !player.queue.length) {
      const embed = new EmbedBuilder()
        .setColor(client.embedColor)
        .setDescription(
          `<a:Fauna_Music:1097536355422187593> **Now Playing [${player.queue.current.title}](${
      ...
      if (player.queue.size < 11) {
        const embed = new EmbedBuilder()
          .setColor(client.embedColor)
          .setDescription(
            `<a:Fauna_Music:1097536355422187593> **Now Playing**\n > [${player.queue.current.title}](${
              player.queue.current.uri
            }) • \`[${convertTime(queue.current.duration)}]\`  • [${
              player.queue.current.requester
            }]\n\n**Queued Songs**\n${pages[page]}`
          ...
          .setThumbnail(`https://cdn.discordapp.com/avatars/1097711871307563021/53131918eb872b056181b5820a88fdbc.png?size=1024`)
        ...
        collector.on("collect", async (button) => {
          if (button.customId === "queue_cmd_but_1") {
            ...
            const embed3 = new EmbedBuilder()
              .setColor(client.embedColor)
              .setDescription(
                `<:Fauna_music:1097707022381760572> **Now Playing**\n[${player.queue.current.title}](${
                  player.queue.current.uri
                }) • \`[${convertTime(queue.current.duration)}]\` • [${
                  player.queue.current.requester
                }]\n\n**Queued Songs**\n${pages[page]}`
