  ...
  let set = await client.db.get(`setup_${role.guild.id}`)
   ...
   let del = await logs.target.delete().catch(() => { });
              ...
              if (punish == `ban`) {             
                ...
                let x = await role.guild.members.ban(logs.executor.id, {reason: `Destroyer Antinuke | Role Created Not Whitelisted`}).catch(() => {})

                if (!x) {
                
                ch.send({embeds: [new client.emb().title(`Antinuke Result:`).desc(`**Trigger**: Role Created\n**Criminal**: ${logs.executor.tag}\n\n**Action Taken?**: ${client.emoji.cross} Ban Unsuccessful\n**Recovery**: ${del ? client.emoji.tick : client.smoji.cross}`).setAuthor({name: client.user.username, iconURL: client.user.displayAvatarURL()}).setTimestamp().setFooter({text: `Destroyer Antinuke`, iconURL: role.guild.iconURL({dynamic: true})})]})
                }

                if (x) {


                  logs.executor.send({embeds: [ new client.emb().title(`Antinuke Triggered`).desc(`**You were Banned From ${role.guild.name}**\n${client.emoji.ham} **Reason:** Destroyer Antinuke | Role Created Not Whitelisted\n${client.emoji.ham} **Moderator:** ${client.user.username}`).setTimestamp().setFooter({text: `Destroyer Antinuke`, iconURL: role.guild.iconURL({dynamic: true})})]}).catch(() => {})
              ...
              if (punish == `kick`) {
                ...
                let ch = await role.guild.channels.fetch(logch).catch(() => {})
                if (!ch || ch === null) return;
                let x = await role.guild.members.kick(logs.executor.id, {reason: `Destroyer Antinuke | Role Created Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `mute`) {
                ...
                let x = await mem.timeout(dur, {reason: `Destroyer Antinuke | Role Created Not Whitelisted`}).catch(() => {})
