    ...
    if (
      ...
      await message.member.timeout(900000)
        .then(() => {
          ...
          ch.send({embeds: [new client.emb().desc(`**${client.emoji.warn} Member Muted**\n**Member**: ${message.author.tag}\n**Member Id**: ${message.author.id}\n\n**Reason**: Posing Links`).setAuthor({name: `Automod Action`, iconURL: message.author.displayAvatarURL({dynamic: true})}).setFooter({text: `Destroyer Automod`, iconURL: client.user.displayAvatarURL()}).setTimestamp()]}).catch(() => { })
