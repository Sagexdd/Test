  ...
  client.on('messageCreate', async (message) => {
    ...
    if (newLines >= 4 || caps >= 7) {
      ...
      message.channel.send({
          ...
          ch.send({embeds: [new client.emb().desc(`**${client.emoji.warn} Member Muted**\n**Member**: ${message.author.tag}\n**Member Id**: ${message.author.id}\n\n**Reason**: Mass Caps/Lines`).setAuthor({name: `Automod Action`, iconURL: message.author.displayAvatarURL({dynamic: true})}).setFooter({text: `Destroyer Automod`, iconURL: client.user.displayAvatarURL()}).setTimestamp()]}).catch(() => { });
